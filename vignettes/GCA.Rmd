...
```{r echo = FALSE, fig.cap = 'Figure 1: An overview of connectedness statistics implemented in the GCA R package.', out.width = '90%' }
knitr::include_graphics("GCstat.png")
# 
# library(DiagrammeR)
...
```{r}
dfPEVD <- data.frame("PEVD" = PEVD_GrpAve, 'Prediction accuracy' = PASum, 
"Scenario" = paste('S', 1:5, sep = ''), check.names = FALSE)
dfCD <- data.frame("CD" = CD_GrpAve, 'Prediction accuracy' = PASum, 
"Scenario" = paste('S', 1:5, sep = ''), check.names = FALSE)
p1 <- ggplot(dfPEVD,  aes(PEVD, `Prediction accuracy`)) + geom_point(aes(shape = factor(Scenario)), color = "blue", size = 3) + 
  xlab("PEVD") + labs(shape="Scenario")
p2 <- ggplot(dfCD,  aes(CD, `Prediction accuracy`)) + geom_point(aes(shape = factor(Scenario)), color="red", size = 3) + 
  labs(shape = "Scenario")
